...
import { useGetAllDiarysQuery } from '../../store/apiSlice';
import UserImg from '../../assets/user.png';

const DiaryScreen = ({ useremail }) => {
    const { data, error, isLoading } = useGetAllDiarysQuery();
    ...
    return (
        <Container>
            <FlatList
                ...
                renderItem={({ item }) => (
                    <>
                        {useremail === item.userData.useremail && (
                            <PostBox>
                                <PostProfileImgBox>
                                ...
                                <PostDataBox>
                                    ...
                                    {item.imgData && (
                                        <PostImgBox>
                                            <PostImg
                                                source={{
                                                    uri: `http://localhost:3000/diaryImages/${item.imgData.filename}`,
                                                }}
                                            />
                                        </PostImgBox>
                                    )}
                                    {item.DiaryData && <PostText> {item.DiaryData.text} </PostText>}
                                </PostDataBox>
                            </PostBox>
                        )}
                    </>
